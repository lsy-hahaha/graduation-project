...
for l = 1:L
    ...
    %在cell中均匀随机地放入K个UEs。该过程是迭代的，因为不满足最小距离的终端被替换为新的终端。
    while perBS(l)<K
    ...
    for j = 1:L
        ...
        %计算单元l中终端到BS j的距离，采用绕圈拓扑，其中考虑终端与BS的9个不同位置之间的最短距离
        ...
        %采用基于3GPP模型的大规模衰落模型计算平均信道增益，忽略各终端的阴影衰落
        for k = 1:K
    ...
    for k = 1:K
        ...
        %Store average channel gains with shadowing fading 平均信道增益
    ...
    for j=1:L
        ...
        for k=1:K
            ...
            %计算单元l中UE k与BS j之间的名义角，利用局部散射模型生成通道的空间相关矩阵
