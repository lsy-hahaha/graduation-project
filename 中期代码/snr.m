       ...
       %生成信道
        ...
        for j = 1 : L
            ...
            for k = 1 : i_ant
                ...
                sum_e_H(1,4) = sum_e_H(1,3) + (H(k,(j-1)*L*K+(j-1)*K+1:(j-1)*L*K+j*K)-H_8(k,(j-1)*K+1:j*K)) * (H(k,(j-1)*L*K+(j-1)*K+1:(j-1)*L*K+j*K)-H_8(k,(j-1)*K+1:j*K))';
            end
...
semilogy(SNR,mse_store(:,3),'g-d','MarkerSize',8)
semilogy(SNR,mse_store(:,4),'k-d','MarkerSize',8)
semilogy(SNR,mse_store(:,5),'-r*','MarkerSize',8)
grid on ;
legend('LS','EVD','SVD','文献8','CRB','Location','Northeast');
