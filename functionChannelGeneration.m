...
for k=1:K
    for l=1:L
        for j=1:L
            ...
            if probLOS(k,l,j)==1 %The LoS Path exists, Rician Factor ~= 0 存在LOS
...
%Generate uncorrelated random variables 生成不相关的随机变量
W = (randn(M,nbrOfRealizations,K,L,L)+1i*randn(M,nbrOfRealizations,K,L,L));

%Prepare to store channel realizations 准备存储
